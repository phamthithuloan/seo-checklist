...
export default function HistoryList({ refreshKey = 0, onOpen }: Props) {
  ...
  if (items === null && !error) {
    return (
      <div className="rounded-2xl bg-white dark:bg-slate-900 shadow-soft ring-1 ring-slate-200/70 dark:ring-slate-700/70 p-10 text-center text-sm text-slate-500 dark:text-slate-400">
        Đang tải lịch sử…
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="rounded-2xl bg-rose-50 dark:bg-rose-900/30 ring-1 ring-rose-200 dark:ring-rose-700 p-6 text-sm text-rose-700 dark:text-rose-300">
        {error}
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl bg-white dark:bg-slate-900 shadow-soft ring-1 ring-slate-200/70 dark:ring-slate-700/70 overflow-hidden">
        ...
        {items.map((it) => (
          ...
          >
            <div className="min-w-0 flex-1">
              ...
              >
                <p className="text-sm font-semibold text-slate-900 dark:text-slate-100 truncate group-hover:text-brand-700">
                  {it.title || it.keyword || "Bài viết không tiêu đề"}
                </p>
                <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-xs text-slate-500 dark:text-slate-400">
                  <span className="inline-flex items-center gap-1">
                    <span className="text-slate-400 dark:text-slate-500">Từ khoá</span>
            ...
            <div className="flex items-center gap-3 shrink-0">
              ...
              <button
                ...
                className="p-1.5 rounded-lg text-slate-400 dark:text-slate-500 hover:text-rose-600 hover:bg-rose-50 transition disabled:opacity-50"
                aria-label="Xoá"
                title="Xoá"
              >
